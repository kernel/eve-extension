...
1. **Open a browser.** Call `manage_browsers` with `action: "create"` (use `stealth: true` for real sites, keep `headless: false` — the default — so the live view stays available for hand-off, and set `timeout_seconds` to `600` (10 min), raising it only when you expect a longer task). Keep the returned `session_id` — every other Kernel call needs it — and the `browser_live_view_url`: share it early so a human can watch or step in. Pass a `start_url` when you already know where to begin.
...
- **Take-over via live view.** Every headful session has a `browser_live_view_url` (from `manage_browsers`; it's absent in `headless` mode, so create with `headless: false` when hand-off may be needed). A human can open it and drive the browser directly — move the mouse, type, click, clear a prompt — at any moment. Share it proactively. When you resume, **re-read the page first** (they may have changed the state) and continue from wherever they left it.
...
1. Check `manage_auth_connections` for an existing connection for that domain. If one is authenticated, reuse it — but managed auth is profile-scoped, so create the browser with that connection's `profile_name`; only then does it start already logged in.
...
- Reuse the same browser — pass the `session_id` from step 1 to every `execute_playwright_code` and `computer_action` call. Never open a second browser; if you've lost the id, recover it with `manage_browsers` (`action: "list"`). If the session isn't in the list — it expired or was deleted — don't keep retrying: report it, and recreate with the same `profile_name` if you need to continue.
